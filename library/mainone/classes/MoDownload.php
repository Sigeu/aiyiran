 ...
 * @version $Id: MoDownload.php 10 2013-08-09 07:32:02Z zhoulifeng $
 * @package utility
 */
 // 使用范例：
...
class MoDownload
{
    public $debug=false;             //启动模式
    public $errormsg='';            //出错信息
    public $Filter=array();         //过滤下载类型数组
    public $filename='';            //要下载的当前文件名
    public $mineType='text/plain';  //文件媒体类型
    public $xlq_filetype=array();   //所有允许的文件类型
    ...
    function downloadfile($filename,$name = "")
    {
        ...
        if($this->fileCheck())
        {
            $arra = pathinfo($this->filename);
            if(empty($name))
                $fn =  $arra['basename'];
			else
                $fn = $name.".".$arra['extension'];
    ...
    function fileCheck()
    {
        ...
        if(file_exists($filename))
        {
            ...
            $filetype = strtolower($file_info['extension']);//取得文件后缀名
            if(!in_array($filetype,$this->Filter))//是否在允许的文件类型里面
            {
                $this->errormsg .= '不被允许的下载类型！';
