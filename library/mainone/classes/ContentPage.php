...
class ContentPage {
	...
	public $total;  //定义返回的字符
	public function __construct() {
		//定义HTML数组
		$this->html_tag = array ('p', 'div', 'h', 'span', 'strong', 'ul', 'ol', 'li', 'table', 'tr', 'tbody', 'dl', 'dt', 'dd', 'em', 'td', 'u', 'sub', 'sup', 'strike');
		$this->html_end_tag = array ('/p', '/div', '/h', '/span', '/strong', '/ul', '/ol', '/li', '/table', '/tr', '/tbody', '/dl', '/dt', '/dd', '/em', '/td', '/u', '/sub', '/sup', '/strike','br', 'br/');
		...
		if (strpos($content, '<')!==false) {
			...
			foreach ($content_arr as $t => $c) {
				...
				if ($c) {
                    ...
                    /**
                     * 原方法当内容为纯字符串时有问题，暂时修改为：
                     */
                    //$this->content .= $this->separate_content($content, $maxwords); //纯文字时
                    if (mb_strlen($content, 'utf8') <= $this->surplus) {
                        $this->data[] = $content;
                    } else {
                        $len = mb_strlen($content, 'utf8');
                        $start = 0;
                        while ($len > $maxwords) {
                            $this->data[] = mb_substr($content, $start, $maxwords, 'utf8');
                            $start += $maxwords;
                            $len -= $maxwords;
                        }
                        if ($len > 0) {
                             $this->data[] = mb_substr($content, $start, $len, 'utf8');
                        }
                    }
		}
		return implode('[page]', $this->data);
	...
	private function separate_content($str = '', $max, $tag = '', $t = 0, $n = 1, $total = 0) {
		...
		if ($str) {
			...
			if (strlen($str)>$this->surplus) { //当前字符数超过最大分页数时
				$remove_str = csubstr($str, $this->surplus, '');
