 ...
 * @Last Modified time: 2014-06-05 13:32:39
 */

// namespace vendor;
	if (!class_exists('ajaxPage')){
    require_once(DIR_BF_ROOT."classes/ajaxPage.php");
}
...
class ajaxPage{
	 ...
	 */
	public function __construct($total,$row=1,$pageNum=10, $isAjax = true){
		$this->total = $total;
		$this->row = $row;
	...
	private function getPre(){
		if($this->selfPage>1){
			return "<a href='".$this->url.($this->selfPage-1)."' class='prav'><i></i>上一页</a>";
		}else{
			return "<a href='#' class='prav'><i></i>上一页</a>";
		}
	}
	...
	private function getNext(){
		if($this->selfPage<$this->totalPage){
			return "<a href='javascript:;' onclick='next($this->selfPage+1);' class='next'>下一页<i></i></a>";
			// return "<a href='".$this->url.($this->selfPage+1)."' class='next'>下一页<i></i></a>";
		}else{
			return "<a href='#' class='next'>下一页<i></i></a>";
		}
	}
	...
	private function getLast()
	{
		...
		if($this->selfPage==$this->totalPage){
			...
			return "<a href='".$this->url.$this->totalPage."'>尾页</a>";
		}
	}
