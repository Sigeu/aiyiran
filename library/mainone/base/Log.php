...
class Log extends Base {
	 ...
	 * @return boolean
	 */
	public static function write($message, $level = 'Error',
			$logFileName = null) {
		...
		error_log(
				date('[Y-m-d H:i:s]')
						. " {$level}: {$message} IP: {$_SERVER['REMOTE_ADDR']}\r\n",
				3, $logFileName);
	}
	...
	public static function show($logFileName = null) {
		...
		foreach ($listStrArray as $key => $linesStr) {
			...
			echo '<tr><td height="25" align="left" bgcolor="' . $bgColor
					. '">&nbsp;' . $linesStr . '</td></tr>';
		}
	...
	protected static function getLogFile($logFileName = null) {
		return DIR_LOG . date('Y-m') . '/'
				. (is_null($logFileName) ? date('Y-m-d') : $logFileName)
				. '.log';
	}
}
