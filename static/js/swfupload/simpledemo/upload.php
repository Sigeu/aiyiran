...
// Settings
	$save_path =  "/static/uploadfile/ckedit/";
	$upload_name = "upload";
...
// Validate file extension
	...
	if (!$is_valid_extension) {
		HandleError("只允许上传jpg,gif,png,bmp的图片");
		exit(0);
	}
...
// Process the file
	...
	*/
	$now = time();
	$file_name = $now . '.' .$path_info['extension'];
	if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $_SERVER['DOCUMENT_ROOT'].$save_path.$file_name)) {
		HandleError("File could not be saved.");
		exit(0);
	}else{
	    mkhtml(2,$config['site_url'].$save_path.$file_name,'');
