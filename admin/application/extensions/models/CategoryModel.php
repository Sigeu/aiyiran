...
class CategoryModel extends Model
{
	 ...
	 * @return array()
	 */
	public function getChildIdList($pid=0)
	{
		$data = $this->findAll(array('pid'=>$pid),'id ASC','id,pid,catname,created');
	 ...
	 * @return array()
	 */
	public function getChildIdList2($pid=0)
	{
		$data = $this->findAll('pid != ' . $pid,'id ASC','id,pid,catname,created');
	...
	public function getContentIdList() {
		$sql = 'SELECT DISTINCT(`' . $this -> tablePrefix . 'maintable`.id) content_id , `' . $this -> tablePrefix . 'maintable`.categoryid categoryid, `' . $this -> tablePrefix . 'maintable`.title title, `' . $this -> tablePrefix . 'maintable`.created created
				FROM `' . $this -> tablePrefix . 'maintable` LEFT JOIN `' . $this -> tablePrefix . 'category`
				ON ' . $this -> tablePrefix . 'maintable.categoryid = ' . $this -> tablePrefix . 'category.id
				LEFT JOIN `' . $this -> tablePrefix . 'model`
				ON ' . $this -> tablePrefix . 'category.model = ' . $this -> tablePrefix . 'model.id 
				WHERE ' . $this -> tablePrefix . 'model.id !=2';
	...
	{
		$data = $this->findAll(array('id'=>$id),null,'id,pid,catname,created');
