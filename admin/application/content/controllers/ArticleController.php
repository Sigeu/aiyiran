...
// 		dump($this->tagModel);exit;
	...
	{
		$urlArr = array(
				'indexUrl' => $this->createUrl('content/Article/index/page/'.self::$page),
				'addUrl' => $this->createUrl('content/Article/add/page/'.self::$page),
				'updateUrl' => $this->createUrl('content/Article/update/page/'.self::$page),
				'detailUrl' => $this->createUrl('content/Article/detail/page/'.self::$page),
				'replaceUrl' => $this->createUrl('content/Article/replace/page/'.self::$page),
				'delUrl' => $this->createUrl('content/Article/del/page/'.self::$page),
				'columnUrl' => $this->createUrl('content/Column/index'),
				'moveHtmlUrl' => $this->createUrl('content/Article/moveCategory'),//移动页面
				'moveUrl' => $this->createUrl('content/Article/move/page/'.self::$page),//移动处理
                'updateorderUrl' => $this->createUrl('content/Article/updateorder/page/'.self::$page),
		);
		$search = array();
		...
		//判断弹出层的权限
		$hasper = 1;
		if(!in_array('/content/article/movecategory',$_SESSION['alllinks']))
		{
			$hasper = 1;
		}
		else
		{
			if(!in_array('/content/article/movecategory',$_SESSION['mylinks'])&&$_SESSION['roleid']!=1)
			{
				$hasper = 0;
			}
		...
		$this->display("content/article/index");
	}
	/**
	 * 添加内容
	 */
	public  function addAction()
	{
		...
		$urlArr = array(
			'indexUrl' => $this->createUrl('content/Article/index/page/'.self::$page),
			'addsaveUrl' => $this->createUrl('content/Article/addsave/page/'.self::$page),
			'indexUrl'=>$this->createUrl('/content/Article/index/page/'.self::$page)
		);
		...
		$this->display('content/article/add');
	}
	/**
	 * 保存添加内容
	 */
	public  function addsaveAction()
	{
		...
		admin_log('添加文章', '添加了文章'.$mainArr['title']);


		if(!$maintalbeId)
		{
			$this->dialog('/content/Article/index/page/'.self::$page,'error','添加失败');
		}
		...
		//搜索关键词接口
		$this->searchModel->searchAdd(self::$modelid,$mainArr['categoryid'],$maintalbeId,$mainArr['title'],$mainArr['description'],$content['content'],$mainArr['keywords'],$mainArr['created']);


		if(!$contentId)
		{
			$this->dialog('/content/Article/index/page/'.self::$page,'error','附表内容添加失败');
		}
		else {
...
document.write( "<img src=\'' .HOST_NAME . 'content/Content/index/id/' . $maintalbeId  . '\' style=\'display:none\'/>" );//访问动态地址
            ...
            $this->dialog('/content/Article/index/page/'.self::$page,'success','添加成功');
		}
	}
	...
	public function delAction()
	{
		...
		$mid = Controller::getParams('mid') ? Controller::getParams('mid') : self::$modelid;
		...
		if($_SESSION['roleid'] == 1)
		{
			...
			foreach($array as $key=>$value) //循环删除多表数据
			{
				...
				//Tag标签接口
				$this->tagModel->deleteTags($mid[0],$delstr);//（模型id,信息id）删除tag标签表中数据
				//搜索接口
				$this->searchModel->searchDel($mid[0],$delstr);//（模型id,信息id）删除tag标签表中数据

			}
			$count = count($ids);
			$this->dialog('/content/Article/index/page/'.self::$page.'/keywords/'.self::$keywords.'/categoryid/'.self::$categoryid.'/order/'.self::$order.'/desc/'.self::$desc,'success',"成功删除{$count}条内容");
		}
		else
		{
			...
			if($nocount)
			{
				$this->dialog('/content/Article/index/page/'.self::$page.'/keywords/'.self::$keywords.'/categoryid/'.self::$categoryid.'/order/'.self::$order.'/desc/'.self::$desc,'info',"对不起，您没有此操作权限");
			}
			else
			{
				foreach($array as $key=>$value) //循环删除多表数据
			    {
					...
					foreach ($arrid as $row2)
					{
						//Tag标签接口
						$this->tagModel->deleteTags(self::$modelid,$row2);//（模型id,信息id）  删除tag标签表中数据
						//搜索接口
						$this->searchModel->searchDel(self::$modelid,$row2);//（模型id,信息id） 删除搜索表中数据
					}
					$this->dialog('/content/Article/index/page/'.self::$page.'/keywords/'.self::$keywords.'/categoryid/'.self::$categoryid.'/order/'.self::$order.'/desc/'.self::$desc,'success',"成功删除{$okcount}条内容 ");
			    }
			}
...
// 				dump($content);exit;
				...
				if(!$flag)
				{
					$this->dialog('/content/Article/index/page/'.self::$page,'error','修改失败');
				}
				/*附表内容*/
				$tablename = M('model')->field('tablename')->where(array('id'=>self::$modelid))->getOne();
				$flag2 = M($tablename['tablename'])->update(array('maintable_id'=>$id),$content);
				if(!$flag2)
				{
					$this->dialog('/content/Article/index/page/'.self::$page,'error','附表内容修改失败');
				}
				else
				{
					//Tag标签接口
                    ...
                    //更新静态文件
                    if($mainArr['publishopt'] == 1) {  //修改生成静态文件
                        ...
                        for ($i=1;$i<=$countnum;$i++) {
                            ...
                            $temp_path = HOST_NAME . "content/Content/index/id/$id/cpage/$i/up_click/0";
                            echo $static . '<script type="text/javascript">
document.write( "<img src=\'' .HOST_NAME . 'content/Content/index/id/' . $id  . '\' style=\'display:none\'/>" );//访问动态地址
					...
					admin_log('修改文章', '修改了文章'.$mainArr['title']);
					$this->dialog('/content/Article/index/mid/'.self::$modelUrl.'/page/'.self::$page,'success','修改成功');
				}
			}
			else
			{
				$urlArr = array(
					'updateSaveUrl' =>  $this->createUrl('content/Article/update/page/'.self::$page),
				);
				...
				$this->assign('urlArr',$urlArr);
				$this->assign('formvalidator',$formvalidator);
				$this->display('content/article/update');
	...
	public function moveCategoryAction()
	{
		...
		$this->assign('per',$per);
		$this->display('content/article/category_move.html');
	}
	/**
	 * 移动文章
	...
	public function moveAction()
	{
		...
		if($_SESSION['roleid'] == 1)//管理员不用判断权限
		{
			...
			if($okcount>0)
			{
				...
				$this->dialog('/content/Article/index/page/'.self::$page.'/keywords/'.self::$keywords.'/categoryid/'.self::$categoryid.'/order/'.self::$order.'/desc/'.self::$desc,'success',"成功移动{$okcount}条内容 {$message}");
			}
			else
			{
				$this->dialog('/content/Article/index/page/'.self::$page.'/keywords/'.self::$keywords.'/categoryid/'.self::$categoryid.'/order/'.self::$order.'/desc/'.self::$desc,'info',"{$message}");
			}
		}
		else
		{
			...
			if(count($noarrcid)>0)
			{
				$this->dialog('/content/Article/index/page/'.self::$page.'/keywords/'.self::$keywords.'/categoryid/'.self::$categoryid.'/order/'.self::$order.'/desc/'.self::$desc,'info',"对不起，你没有此操作权限");
			}
	...
	public  function previewAction()
	{
		...
		$this->assign('form',$form);
		$this->assign('formvalidator',$formvalidator);
		$this->display('content/article/preview');
	}
	/**
	 * 预览表单变化
	*/
	...
	public function replaceAction()
	{
		...
		{
			$urlArr = array(
				'changeCategory' => $this->createUrl('content/Article/changeCategory'),
			);
			...
			$this->assign('urlArr',$urlArr);
			$this->display('content/article/replace');
	...
	public function changeCategoryAction()
	{
		...
		$html = $this->fetch('content/article/categoryForm.html');
		exit($html);
	}
	...
	function updateorderAction ()
	{
		...
		$this->dialog('/content/Article/index/mid/'.self::$modelUrl .'/page/'.self::$page.'/keywords/'.self::$keywords.'/categoryid/'.self::$categoryid.'/order/'.self::$order.'/desc/'.self::$desc,'success',"操作成功");
	}
