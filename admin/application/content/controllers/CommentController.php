 ...
 *
 * 文件修改记录：
 * <br>雷少进  2013-01-16 15:15 创建此文件
 * <br>雷少进  2013-02-18 10:30 修改此文件
 *
...
class CommentController extends AdminController
{
	...
	{	
		...
		//搜索
		$search['title']     = isset($_POST['title']) ? $_POST['title'] : (isset($_GET['title']) ? $_GET['title'] : (isset($_SESSION['search']['title'])?$_SESSION['search']['title']:''));
		$search['modelid']   = isset($_POST['modelid']) ? $_POST['modelid'] : (isset($_GET['modelid']) ? $_GET['modelid'] : (isset($_SESSION['search']['modelid'])?$_SESSION['search']['modelid']:'1') );
		$search['status']    = isset($_POST['status']) ? $_POST['status'] : (isset($_GET['status']) ? $_GET['status'] : (isset($_SESSION['search']['status'])?$_SESSION['search']['status']:'') );
		$search['is_reply']  = isset($_POST['is_reply']) ? $_POST['is_reply'] : (isset($_GET['is_reply']) ? $_GET['is_reply'] : (isset($_SESSION['search']['is_reply'])?$_SESSION['search']['is_reply']:'') );
		

		$this -> assign('pflag',$pflag);
			
		$plist = $this -> getCommentModel() -> getCommentList($search);
		$_SESSION['search'] = $search;//查询条件SESSION
		
		$this -> assign('search',$search);
		$this -> assign('plist',$plist);
		$this -> assign('tab',isset($_REQUEST['tab']) ? $_REQUEST['tab'] : '1');
		
		$this -> display('content/comment/content_comment_index.html');
	}
	...
	function passfailAction ()
	{
		...
		admin_log('审核评论', '审核了评论('.$this -> getCmtTitleByIds($ids).')的评论状态为'.$this -> getStatus($status));
		$this -> dialog('/content/comment/index');
	}
	...
	public function delAction ()
	{
		...
		admin_log('删除评论', '删除了评论('.$title_str.')');
		
		if ($pflag == 1) {
			$ext_str = '/tab/1';
		} elseif ($pflag == 2) {
			$ext_str = '/status/' . $_SESSION['search']['status'];
		} elseif ($pflag == 3) {
			$ext_str = '/is_reply/' . $_SESSION['search']['is_reply'];
		}
		$this -> dialog('/content/comment/index' . $ext_str);
	}
	...
	public function viewAction ()
	{
		...
		if(isset($_POST['comment_id']) && $_POST['comment_id'])
		{
			...
			if($res)
				$this -> dialog('/content/comment/index','success','操作成功');
			else
				$this -> dialog('/content/comment/index','success','操作失败');
		}
		...
		$this -> assign('search' , $_SESSION['search']);
		$this -> display('content/comment/content_comment_view.html');
	}
	...
	public function replyAction ()
	{
		...
		if(isset($_POST['comment_id']) && $_POST['comment_id'])
		{
			...
			if($res)
				$this -> dialog('/content/comment/index' . $ext_str,'success','操作成功');
			else
				$this -> dialog('/content/comment/index' . $ext_str,'error','操作失败');
		}
		...
		$this -> assign('info',$info);
		$this -> assign('search' , $_SESSION['search']);
		$this -> display('content/comment/content_comment_reply.html');
	}
